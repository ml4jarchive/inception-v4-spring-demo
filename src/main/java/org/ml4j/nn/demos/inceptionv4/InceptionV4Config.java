...
public class InceptionV4Config {
	...
	DirectedComponentFactory directedComponentFactory() {
		// Decorate the directed component factory with diagnostics
		DefaultDirectedComponentFactoryImpl decorated =  
				new DefaultDirectedComponentFactoryImpl(matrixFactory(), axonsFactory(), activationFunctionFactory());
		DirectedComponentFactory adapter = new DirectedComponentFactoryAdapter(decorated);
		decorated.setDirectedComponentFactory(adapter);
		return adapter;
	}
